...
##########                            Init                            ##########
################################################################################
com.ID      <- snakemake@config[['com_id']]
com.ID.list <- str_split(com.ID, ',', simplify = T)[1,]

source(file.path('./code/shared_functions.R'))

dir.proj <- paste0('.')

com.ID      <- snakemake@config[['com_id']]
dir.outs.qc <- snakemake@config[['qc_path']]
dir.bcls <- snakemake@config[['bcl_path']]


for (com.ID in com.ID.list) {
  dir.outs.log <- file.path(snakemake@config[['log_path']], com.ID)
  ...
  var.wofl <- snakemake@config[['workflow']]
  var.vers <- snakemake@config[['chemistry']]
  var.orga <- snakemake@config[['organism']]
  
  pool.table <- read.table(file.path(dir.outs.log,'poolTable-updated.csv'))
  ...
  foreach(i = seq(nPools),
    ...
    sink.file <- file(paste0(snakemake@config[['log_path']], '/04_make-seurat_',pool.10x,'.log'), 'a')
    ...
    cat(rep('#',80),'\n',
        ...
        '##\n',
        '##\n',
        '##\tPROJECT ID:     ',snakemake@config[['scop_id']],'\n',
        '##\n',
        '##\tCOM ID:         ',com.ID,'\n',
        '##\n',
        '##\tORGANISM:       ',var.orga,'\n',
    ...
    mat.files.10x <- file.path(snakemake@config[['out_path']],com.ID,'10x',pool.10x,'res')
    if (!file.exists(mat.files.10x)) { stop('Not able to locate quants_mat.gz (gene expression)')}
    
    if (var.wofl == '10x + HTO') {
      mat.files.hto <- file.path(snakemake@config[['out_path']],com.ID,'hto',pool.hto,'res')
