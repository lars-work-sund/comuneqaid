####################  Start sink  ####################
sink.file <- file(file.path(snakemake@log[[1]]), 'w')
...
if (var.wofl == '10x') {
  pool.table <- read.csv(file.path(dir.outs.log,'poolTable.csv'),
...
for (pool.i in seq(n.pools)) {
  ...
  for (bcl in rev(tmp.seqs.all)) {
    stats.json.path <- file.path(snakemake@config[['fastq_path']], bcl, 'fastq/Stats/Stats.json')
    ...
    for (pool.10x.i in seq(pool.10x)) {
      ...
      if (var.vers == 'RNA v3.0') {
        ...
        tmp.pool[['Index (10x)']] <- paste(pool.10x, collapse = ',')
      }
      if (var.vers == 'RNA v3.1') {
        tmp.index <- pool.10x[pool.10x.i]
        tmp.pool[['Index (10x)']] <- paste(pool.10x, collapse = ',')
      }
...
write.table(x = pool.table,
            file = file.path(dir.outs.log,'poolTable-updated.csv'))
